...
loginprotect_doc_files=LICENSE README.md
loginprotect_src_dirs=appinfo vendor l10n
loginprotect_all_src=$(loginprotect_src_dirs) $(loginprotect_doc_files)
build_dir=build
...
#
# ownCloud loginprotect PHP dependencies
#
...
$(dist_dir)/loginprotect: $(composer_deps)
	rm -Rf $@; mkdir -p $@
	cp -R $(loginprotect_all_src) $@
...
.PHONY: dist
dist: clean $(dist_dir)/loginprotect
ifdef CAN_SIGN
